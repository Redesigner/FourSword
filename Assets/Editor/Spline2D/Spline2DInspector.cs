...
public class Spline2DInspector : UnityEditor.Editor {
	...
	private void ShowPoint (int index) {
		...
		if (selectedIndex == index) {
			EditorGUI.BeginChangeCheck();
			var fmh_210_42_638989926599384728 = Quaternion.identity; point = Handles.FreeMoveHandle(point, size * 1.5f * handleSize, Vector3.zero, Handles.CircleHandleCap);
			if (EditorGUI.EndChangeCheck()) {
				Undo.RecordObject(spline, "Move Point");
